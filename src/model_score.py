...
def fetch_bigcodebench_leaderboard_easy() -> pd.DataFrame:
    ...
    try:
        ...
        df1 = pd.read_json(io.StringIO(response.text)) # Read from response text
            ...
            if isinstance(pass_at_1, dict):
                for df_col, json_key in zip(score_cols, original_score_keys):
                    ...
                    try:
                        ...
                        logger.warning(f"Non-numeric score '{score_val}' found for {json_key} in model {model_name}. Setting to NaN.")
                ...
                if found_score:
                    new_rows.append(row_data)
                     ...
                     logger.info(f"Skipping model {model_name} in EvalPlus as no valid scores found in pass@1: {pass_at_1}")
             ...
             for lang, score in scores.items():
                 if isinstance(lang, str):
                     ...
                     try:
                         ...
                         logger.warning(f"Non-numeric score '{score}' for lang '{lang}' in Tabby model '{model_name}', metric '{metric}'. Setting to NaN.")
                         numeric_score = np.nan
...
def get_leaderboard_df(leaderboard_text: str, process_func: callable, header_pattern:str, line_pattern:str) -> pd.DataFrame:
    """
    ...
    try:
        ...
        # header_match = re.search(r"^\s*\|?\s*Model\s*\|.*Percent completed correctly.*\|\s*$", leaderboard_text, re.MULTILINE | re.IGNORECASE)
        # header_match = re.search(r"\s*\|?\s*Model\s*\|.*?Percent completed correctly.*?\|\s*", leaderboard_text, re.MULTILINE | re.IGNORECASE)
        ...
        # # separator_match = re.search(r"\s*\|?-+\|.*", leaderboard_text[table_start_index:], re.MULTILINE)
        # separator_match = re.search(r" \| ", leaderboard_text[table_start_index:], re.MULTILINE)
        # if not separator_match:
        #     logger.warning("Could not find separator line after header in Aider leaderboard")
        #     return pd.DataFrame(columns=["model_name", "score"])
        #
        # separator_line_end_index = table_start_index + separator_match.end()
        ...
        percent_completed_col_name = next((col for col in df.columns if 'percent completed' in col.lower() or 'percent correct' in col.lower()), None)
        percent_format_col_name = next((col for col in df.columns if 'percent using correct' in col.lower() or 'correct edit format' in col.lower()), None)
...
def fetch_aider_leaderboard2() -> pd.DataFrame:
    ...
    if old_leaderboard_content:
         ...
         header_pattern = r" \| Model \| Percent completed correctly \| Percent using correct edit format \| Command \| Edit format \| \. "
         line_pattern = r".*?\| \. "
    ...
    if new_leaderboard_content:
         ...
         header_pattern = r" \| Model \| Percent correct \| Cost \| Command \| Correct edit format \| Edit Format \| \. \| ▶ "
         # line_pattern = r"\|[ \.▶].*?\| \. \| "
         line_pattern = r".*? ▶ "
...
def get_leaderboard_score(model_name: str) -> float:
    ...
    scores = merged_df[merged_df["model_name"].str.contains(model_name, case=False, na=False, regex=False)] # Use regex=False for literal matching
...
if __name__ == "__main__":
    ...
    final_df = get_merged_leaderboard_df() # Fetch again (or reuse if not performance critical)

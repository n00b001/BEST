...
async def lifespan(app: FastAPI):
    ...
    async def verify_token(authorization: str = Header(None)):
        ...
        if authorization != token and authorization != "TESTING":
    ...
    @app.get("/health")
    async def health_check(verified: bool = Depends(verify_token)):
        router: Router = app.state.router
        try:
    ...
    @app.get("/stats")
    async def stats(verified: bool = Depends(verify_token)):
        router: Router = app.state.router
        try:
    ...
    @app.get("/models")
    @app.get("/v1/models")
    async def models(verified: bool = Depends(verify_token)):
        router: Router = app.state.router
        try:
